...
function saveDiagrammonDB(idWorkflow){
    ...
    //richiamo il servizio che salva nel DB 
    var ipAdress="aose.pa.icar.cnr.it";
	var port="8080";
	 $.ajax({
	    url : "http://"+ipAdress+":"+port+"/WorkflowWebEditor/SaveDiagrammDB",
//		url : "http://"+ipAdress+"/WorkflowWebEditor/SaveDiagrammDB",
	    type: "POST",
	    data:{"workFlowData":graphdate,"idWorkflow":idWorkflow},
	    dataType: 'text',
	    success : function (data) {
	   	console.log("ID FILE SAVED INTO REPOSITORY:");
	   	//CLOSE THE WINDOWS????
	   	window.close();
		//window.open("http://"+ipAdress+"/WorkflowWebEditor/goalsSpecFromBPMN.jsp?idFileBPMNSAVED="+data);
	    }
...
function load_workflow_from_repository(idWorkflow){
	var ipAdress="aose.pa.icar.cnr.it";
	var port="8080";
	$.ajax({
	    url : "http://"+ipAdress+":"+port+"/WorkflowWebEditor/LoadWorkflow",
//		 url : "http://"+ipAdress+"/WorkflowWebEditor/LoadWorkflow",
	    type: "GET",
	    data: { idWorkflow:idWorkflow},
	    dataType: 'text',
	    success : function (data) {
	    	
	    	var url="./tmpProcess/tempProcess.json";
...
//	    	console.log("load_workflow_from_repository:"+data);
...
////	    		
	    ...
	    error : function (richiesta,stato,errori) {
	    	console.log("error in load_workflow_from_repository  ");
	    }
	});
}